            ...
            if Customers.objects.filter(cust_name=receiver).exists():
                if sender != receiver:
                    ...
                    if (int(tamount)<= int(cus.amount)):
                        ...
                        Customers.objects.filter(cust_name=sender).update(amount=int(F('amount'))-int(tamount),transactions=dt)
                        Customers.objects.filter(cust_name=receiver).update(amount=int(F('amount'))+int(tamount))
